...
%><%
    ...
    if(aiframe)
    {
        ...
        {
%>
        <iframe class="ifram_content <%=pid%>" src="<%=_fileName%>?pid=<%=pid%>&iframe=true&id=<%=id%>" frameborder="0" width="100%"></iframe>
<%
...
<%
    ...
    if(id==null)
    {
        ...
        fields=getFields(obj, "gridProps", extProps);
    }else
    {
        extProps=getExtProps(obj,"formExtProps",eng);
        fields=getFields(obj, "formProps", extProps);
    }
...
<%
            if(rid!=null)
            {
                DataObject pobj=eng.getDataSource("Page").fetchObjById(obj.getString("parentId"));
...
%>                
                ...
                {
                    parent.loadContent("<%=_fileName%>?pid=<%=pid%>&id=",".content-wrapper");
                    return false;
                },                                 
<%
                }
...
%>                
            ...
            <%=parseScript(obj.getString("gridAddiJS"),request)%>
<%
            //********************************** End Grid ************************************************************    
        }else
        {
...
%>
            ...
            form.submitButton.click = function (p1)
            {
                eng.submit(form, this, function ()
                {
                    isc.say("Datos enviados correctamente...", function () {
                        <%if(add){%>parent.loadContent("<%=_fileName%>?pid=<%=pid%>&id=" + form.values._id,".content-wrapper");<%}%>
                    });
                });
            };
            ...
            form.buttons.addMember(isc.IButton.create({
                ...
                padding: "10px",
                click: function (p1) {
                    parent.loadContent("<%=_fileName%>?pid=<%=pid%>",".content-wrapper");
                    return false;
                }
            }));
            form.buttons.members.unshift(form.buttons.members.pop());  
            
            <%=parseScript(obj.getString("formAddiJS"),request)%>
<%
            //********************************** End Form ************************************************************    
        }
%>    
